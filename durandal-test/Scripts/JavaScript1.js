...
(function (factory) {
    ...
    var utils = (function () {
        ...
        return {
            ...
            getObservable: function (model, propertyName) {
                var hasProperty = model.hasOwnProperty(propertyName),
                    isGetter = hasProperty ? Object.getOwnPropertyDescriptor(model, propertyName).hasOwnProperty('get') : false,
                    observable = hasProperty ? isGetter ? ko.getObservable(model, propertyName) :
                                                           ko.isObservable(model[propertyName]) ? model[propertyName] : null
                                              : null;
                return ko.isObservable(model) ? model : observable;
            },
            getObservableFromBindingContext: function (element, bindingContext) {
                var element = bindingContext.$element || element,
	            viewModel = bindingContext.$data,
	            hasBindAttribute = utils.hasAttribute(element, 'data-bind'),
	            bindAttribute = utils.getAttribute(element, "data-bind");
                ...
                findObservable = function (accessor, obj) {
                    ...
                    return utils.getObservable(result, objects[objects.lenght - 1]);
                }

                if (hasBindAttribute) {
                    var matchResult = bindAttribute.match(/(validationMessage|validationElement|value)\:[ \t]+([A-z_0-9\-]+)/),
						accessor = matchResult && matchResult.length === 3 ? matchResult[2] : null;
    ...
    //#region ValidationBinder
    var validationBinder = function (viewModel) {

        var internal = new function () {
            var model = viewModel;

            return {
                bind: function (propertyName, options) {
                    ...
                    if (observable) {
                        if (isGetter) {
                            Object.defineProperty(model, propertyName, {
                                configurable: true,
                                enumerable: true,
                                get: observable.extend(options),
                                set: ko.isWriteableObservable(observable) ? observable : undefined
                            });
                        } else {
                            model[propertyName] = observable.extend(options);
                        }
                    }
                    return internal;
                }
    ...
    var api = (function () {
        ...
        return {
            ...
            group: function group(obj, options) { // array of observables or viewModel
                ...
                traverse = function traverse(obj, level, propertyName) {
                    var objValues = [],
                    ...
                    var observable = ko.getObservable(obj, propertyName) || ko.isObservable(obj) ? obj : null;
                    if (observable) {

                        //make sure it is validatable object
                        if (!observable.isValid) { observable.extend({ validatable: true }); }
                        validatables.push(observable);
                    }

                    //get list of values either from array or object but ignore non-objects
                    if (val) {
                        if (utils.isArray(val)) {
                            objValues.push({ value: val, name: propertyName });
                        } else if (utils.isObject(val)) {
                            for (var key in val) {
                                if (val.hasOwnProperty(key)) {
                                    objValues.push({ value: ko.getObservable(obj, key) || val[key], name: key });
                                }
                            }
                        }
                    }
                    ...
                    if (level !== 0) {
                        ko.utils.arrayForEach(objValues, function (item) {
                            var observable = item.value;
                            //but not falsy things and not HTML Elements
                            if (observable && !observable.nodeType) { traverse(observable, level + 1, item.name); }
                        });
                    }
                };
    ...
    ko.bindingHandlers['validationCore'] = (function () {
        ...
        return {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                ...
                // augment binding context with element
                ...
                if (config.insertMessages && utils.isValidatable(valueAccessor())) {
                    ...
                    if (config.messageTemplate) {
                        ...
                        ko.applyBindingsToNode(validationMessageElement, { validationMessage: valueAccessor() }, bindingContext);
                    }
                }
                ...
                if (config.decorateElement && utils.isValidatable(valueAccessor())) {
                    ko.applyBindingsToNode(element, { validationElement: valueAccessor() }, bindingContext);
                }
            },
    ...
    ko.bindingHandlers['validationMessage'] = { // individual error message, if modified or post binding
        update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var obsv = utils.getObservable(element, bindingContext) || valueAccessor(),
